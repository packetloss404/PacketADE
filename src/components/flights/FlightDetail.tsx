...
import { AsyncFlightGrid } from "./AsyncFlightGrid";
...
export function FlightDetail({ flight }: FlightDetailProps) {
  ...
  const isAsyncMode = (flight.attempts?.length ?? 0) > 0;
  ...
  return (
    <div className="flex flex-col flex-1 overflow-hidden bg-bg-primary">
      ...
      {isAsyncMode && <AsyncFlightGrid flight={flight} />}

      {!isAsyncMode && isPlanning && (
