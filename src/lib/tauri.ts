...
/**
 * ACP-only extras for `startApiAgentSession`, grouped rather than appended as
 * two more positional arguments to an already 19-long list.
 *
 * Both are ignored outright by every other transport: the ACP branch of
 * `start_api_agent_session` is the only reader.
 */
export interface StartApiAgentAcpOptions {
  /**
   ...
   */
  inheritEngineMcp?: boolean | null;
  /**
   * An id from the ENGINE's own session directory (`acpListSessions`). Present
   * when this conversation is adopting a session the engine already holds: the
   * backend resumes it with ACP `session/load` instead of `session/new`, so
   * the engine's stored history is the model's context from the first turn.
   *
   * The engine's replay of that history is deliberately NOT rendered — see
   * `agentTaskStore.adoptEngineSession` and `acp/events.rs`.
   */
...
export async function startApiAgentSession(
  ...
  /** ACP transport only; ignored by every other backend. */
  acp?: StartApiAgentAcpOptions | null,
): Promise<void> {
  return invoke("start_api_agent_session", {
    ...
    mcpTrustSnapshot: mcpTrustSnapshot ?? null,
    acpInheritEngineMcp: acp?.inheritEngineMcp ?? null,
    acpEngineSessionId: acp?.engineSessionId ?? null,
  });
}
...
// capability negotiation, session history, model/command/file enumeration,
// MCP disclosure, and usage.
//
// Every payload below mirrors a Rust DTO in `src-tauri/src/acp/mod.rs`, all of
// which are `#[serde(rename_all = "camelCase")]` — the field names here are
// the wire names, not a translation.
//
// Engine lifetime is lazy: `startApiAgentSession` on the `packetcode-acp`
// provider brings the engine up itself, so `acpStart` is only needed by
// surfaces that want an engine BEFORE any conversation exists (a settings
// probe, the MCP disclosure list). Everything that needs a live engine
// rejects with "engine not started" when there is none.

/** Result of `packetcode doctor --json` plus this client's version gate. */
export interface AcpEngineProbe {
  /** Whether an engine binary was found and could be executed at all. */
  found: boolean;
  /** Absolute path of the binary that answered. Absent when none was found. */
  path?: string;
  /** Version the engine reported. Absent when it did not say. */
  version?: string;
  /** The engine's own self-check verdict, verbatim. */
  status?: string;
  /** Oldest engine this client is tested against (`MINIMUM_ENGINE_VERSION`). */
 ...
 *
 * `advertised` is load-bearing: an engine that sent no `_packetcode` object
 * (an older packetcode, or a third-party ACP agent) leaves every boolean
 * `false`, and those falses do NOT mean "feature missing" — the backend's
 * call-time method-not-found fallbacks still decide. Only when `advertised`
 * is true are the booleans authoritative.
 */
export interface AcpPacketcodeCapabilities {
  /** Whether the engine sent an `agentCapabilities._packetcode` object. */
  advertised: boolean;
  /** Gates `acpListSessions` asking the engine (vs its disk fallback). */
  sessionsList: boolean;
  /** Gates `acpRenameSession`. */
  sessionsRename: boolean;
  /** Gates `acpSessionUsage` and prompt-result usage enrichment. */
  ...
  mcpDefaults: boolean;
  /**
   * ACP permission modes `session/new` will accept. Never empty: an engine
   * that advertises nothing (or garbage) yields all five — `ask`,
   * `accept-edits`, `auto`, `read-only`, `bypass`.
   */
  permissionModes: string[];
  /**
   * Mode a `session/new` without an override resolves to, when the engine
   * says. `null` means unknown — the UI must not guess "ask".
   */
  defaultPermissionMode: string | null;
}

/** What the engine advertised in its ACP `initialize` response. */
export interface AcpEngineCapabilities {
  protocolVersion: number;
  /** Spec capability: whether `session/load` may be used to resume. */
  loadSession: boolean;
  /**
   * Spec capability (`sessionCapabilities.close`): the engine can release a
   * session's runtime on request.
   */
  sessionClose: boolean;
  packetcode: AcpPacketcodeCapabilities;
}

/** One persisted engine session, for a resume picker. */
export interface AcpSessionSummary {
  sessionId: string;
  name: string;
  /** ISO timestamp, engine-formatted. The listing is newest-first. */
  updatedAt: string;
  provider: string;
  model: string;
  workingDir: string;
...
/** One provider/model pair the engine can serve. */
export interface AcpModelOption {
  provider: string;
  model: string;
  /** Whether the engine treats this as its default choice. */
  default: boolean;
}

/** One MCP server as the engine reports it. */
export interface AcpMcpServerStatus {
  ...
  command: string;
  /** Startup error text, or "" when there was none. */
  error: string;
}

/** One invocable engine slash command. */
export interface AcpSlashCommand {
  name: string;
  description: string;
  /** "builtin", "user", or "project". */
  source: string;
  /** Short usage tail such as "[arguments]"; absent when it takes none. */
  argumentHint?: string;
}

/** Token/cost usage for one engine session. */
...
/**
 * Locate the engine and check it against this client's minimum version.
 * Never rejects for "not installed" — that is `found: false` on the result.
 * Also remembers the binary that answered, so a later `acpStart` spawns
 * byte-identically what was validated.
 */
export async function acpProbe(): Promise<AcpEngineProbe> {
...
/** Shut the engine down, releasing every session and its MCP children. */
export async function acpStop(): Promise<void> {
  return invoke("acp_stop");
}

/**
 * What the running engine advertised. Before the handshake (or after a stop)
 * this yields the conservative defaults — all five permission modes, nothing
 * advertised — which is exactly how a pre-capability engine is treated, so
 * callers never need a second set of fallbacks.
 */
export async function acpCapabilities(): Promise<AcpEngineCapabilities> {
  return invoke<AcpEngineCapabilities>("acp_capabilities");
}

/**
 * Persisted engine sessions, newest first. Falls back to reading the engine's
 * own `~/.packetcode/sessions/*.json` when the engine predates the extension
 * or disowned it, so this can return rows even with no engine running.
 */
export async function acpListSessions(): Promise<AcpSessionSummary[]> {
  return invoke<AcpSessionSummary[]>("acp_list_sessions");
}
 ...
 */
export async function acpLoadSession(sessionId: string, cwd: string): Promise<void> {
  return invoke("acp_load_session", { sessionId, cwd });
}

/** Provider/model choices the engine can serve. Empty when it cannot say. */
export async function acpListModels(): Promise<AcpModelOption[]> {
  return invoke<AcpModelOption[]>("acp_list_models");
}

/**
 * MCP servers. With a conversation id: that session's LIVE fleet (what
 * actually started, with tool counts and startup errors). Without one: the
 * engine's CONFIGURED servers — the disclosure list, readable before any
 * session exists. Empty when the engine cannot say.
 */
export async function acpListMcpServers(
  sessionId?: string | null,
): Promise<AcpMcpServerStatus[]> {
  return invoke<AcpMcpServerStatus[]>("acp_list_mcp_servers", {
    sessionId: sessionId ?? null,
  });
}

/** Slash commands available in `cwd`. Empty when the engine cannot say. */
export async function acpListCommands(cwd: string): Promise<AcpSlashCommand[]> {
  return invoke<AcpSlashCommand[]>("acp_list_commands", { cwd });
}

/**
 * Project files matching `query`, ranked best-match first by the engine.
 * Paths are project-relative and slash-separated. Empty when the engine
 * cannot say.
 */
export async function acpSearchFiles(cwd: string, query: string): Promise<string[]> {
  return invoke<string[]>("acp_search_files", { cwd, query });
}
...
export async function acpRenameSession(sessionId: string, name: string): Promise<void> {
  return invoke("acp_rename_session", { sessionId, name });
}
...
export interface AcpInstallOutput {
  line: string;
  stream: "stdout" | "stderr";
}

/** Event name carrying streamed installer output. */
export const ACP_INSTALL_OUTPUT_EVENT = "acp:install-output";

/**
 * Install the packetcode engine. Resolves with a fresh probe (already gated on
 * `found && compatible`) or rejects with a human-readable reason.
 *
 * Rejects if the engine is currently running — a live executable cannot be
 * replaced on Windows — so callers should surface "stop the engine first"
 * rather than the raw error. Check `AcpEngineProbe.installSupported` before
 * offering this at all; where it is false, `detail` carries manual steps.
 */
export async function acpInstallEngine(): Promise<AcpEngineProbe> {
  return invoke<AcpEngineProbe>("acp_install_engine");
}

// --- MCP disclosure --------------------------------------------------------

/** Why a server did or did not make it onto the wire. */
export type AcpMcpReason =
  | "trusted"
  | "disabled"
  | "noTrustDecision"
  | "notSelected"
...
export interface AcpMcpPlan {
  ...
  selected: string[];
  /** Inheritance was asked for but the engine never advertised `mcpDefaults`. */
  inheritRefused: boolean;
}
 ...
 *
 * Call after `acpStart()` if you need `inheritRefused` to be meaningful; the
 * other answers need no running engine.
 */
export async function acpMcpPlan(params: {
  projectPath: string;
  enabledMcpServerIds?: string[] | null;
  mcpTrustSnapshot?: unknown[] | null;
  inheritEngineDefaults?: boolean | null;
}): Promise<AcpMcpPlan> {
  return invoke<AcpMcpPlan>("acp_mcp_plan", {
    projectPath: params.projectPath,
    enabledMcpServerIds: params.enabledMcpServerIds ?? null,
    mcpTrustSnapshot: params.mcpTrustSnapshot ?? null,
    inheritEngineDefaults: params.inheritEngineDefaults ?? null,
  });
}
