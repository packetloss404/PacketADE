 ...
 * `import { listProviderModels } from "@/lib/tauri"` call; nothing else in the
 * seam changes.
 */
export async function fetchLiveModels(
  provider: string,
): Promise<LiveModel[] | "unsupported"> {
  const mod: Record<string, unknown> = await import("@/lib/tauri");
  const fn = mod.listProviderModels;
  if (typeof fn !== "function") return "unsupported";
  const result = await (fn as (p: string) => Promise<LiveModel[]>)(provider);
...
export function classifyLiveModelError(error: unknown): {
  status: Extract<LiveModelStatus, "failed" | "unauthorized" | "no-key">;
  message: string;
} {
  const message =
  ...
  if (
    ...
    lowered.includes("missing_key") ||
    lowered.includes("api key not found")
  ) {
    return { status: "no-key", message };
  }
  if (
