...
describe("liveModels — error classification", () => {
  it("separates a missing key, a rejected key, and everything else", () => {
    expect(classifyLiveModelError(new Error("no api key configured")).status).toBe("no-key");
    expect(classifyLiveModelError(new Error("401 Unauthorized")).status).toBe("unauthorized");
  ...
  it("keeps the provider's own message for the surface to show", () => {
    expect(classifyLiveModelError("invalid api key: sk-…abc").message).toContain("sk-");
  });
});
