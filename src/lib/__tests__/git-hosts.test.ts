...
describe("normalizeInstanceBaseUrl, gitea (G2)", () => {
  it("accepts a plain origin", () => {
    expect(normalizeInstanceBaseUrl("gitea", "https://git.example.com")).toEqual({
  ...
  it("requires a scheme", () => {
    expect(normalizeInstanceBaseUrl("gitea", "git.example.com")).toEqual({
      error: "Base URL must start with http:// or https://",
    });
  });

  it("rejects empty input", () => {
    expect(normalizeInstanceBaseUrl("gitea", "   ")).toEqual({ error: "Base URL is required" });
  });

  it("accepts http for a LAN instance", () => {
    expect(normalizeInstanceBaseUrl("gitea", "http://gitea.local:3000")).toEqual({
