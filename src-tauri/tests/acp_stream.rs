...
use packetade_lib::acp::{
    ...
    AcpMcpPosture, AcpMcpServer, AcpState, McpServerStatus, PromptOutcome, SessionUsage,
    PERMISSION_MODES,
};
...
async fn start_mock() -> (Arc<AcpState>, mpsc::UnboundedReceiver<Event>) {
    start_mock_with(&[]).await
}

async fn start_mock_with(
    extra_args: &[&str],
) -> (Arc<AcpState>, mpsc::UnboundedReceiver<Event>) {
...
async fn happy_path_streams_events_in_order() {
    ...
    let session = new_session_on(&state, ".", None, None, None, AcpMcpPosture::None)
        .await
        .expect("session/new");
...
async fn cancel_mid_prompt_yields_cancelled_and_rejects_late_permission_reply() {
    ...
    let session2 = new_session_on(&state, ".", None, None, None, AcpMcpPosture::None)
        .await
        .unwrap();
...
async fn fleet_of(state: &AcpState, session: &str) -> Vec<McpServerStatus> {
...
async fn consented_sessions_inherit_the_engines_mcp_fleet() {
    let (state, _rx) = start_mock_with(&["--mcp", "--mcp-defaults"]).await;
    let session = new_session_on(&state, ".", None, None, None, AcpMcpPosture::InheritEngineDefaults)
        .await
        .unwrap();
    ...
    load_session_on(&state, &session, ".", AcpMcpPosture::InheritEngineDefaults).await.unwrap();
...
async fn sessions_without_consent_start_no_mcp_servers() {
    ...
    load_session_on(&state, &session, ".", AcpMcpPosture::None).await.unwrap();
    assert!(fleet_of(&state, &session).await.is_empty());
...
async fn consent_cannot_omit_mcp_servers_on_an_engine_that_never_promised_it() {
    ...
    load_session_on(&state, &session, ".", AcpMcpPosture::InheritEngineDefaults)
        .await
...
async fn an_explicit_posture_names_exactly_the_trusted_servers() {
    let (state, _rx) = start_mock_with(&["--mcp", "--mcp-defaults"]).await;
    let posture = AcpMcpPosture::Explicit(vec![AcpMcpServer {
        name: "trusted-fs".into(),
        command: "/opt/fs-mcp".into(),
    ...
    let session = new_session_on(&state, ".", None, None, None, posture.clone())
        .await
        .unwrap();
    let live = fleet_of(&state, &session).await;
    // Exactly the client's list: the engine's own three configured servers
    // stay out, because a populated list means "these, nothing else".
    assert_eq!(names(&live), vec!["trusted-fs"]);
    assert_eq!(live[0].source, "client");
    assert_eq!(live[0].command, "/opt/fs-mcp");

    // A resume replays the same frozen fleet rather than re-deriving one.
    load_session_on(&state, &session, ".", posture)
        .await
        .unwrap();
    assert_eq!(names(&fleet_of(&state, &session).await), vec!["trusted-fs"]);

    stop_on(&state).await.unwrap();
}

/// The `mcpDefaults` guard is scoped to the OMISSION, and only to it.
///
/// An explicit list travels in the `mcpServers` field, which every engine
/// understands — including one that predates the capability block entirely.
/// Downgrading it the way an inherit request is downgraded would silently
/// strip a fleet the user explicitly authorized, for no protocol reason.
#[tokio::test]
async fn an_explicit_posture_survives_an_engine_without_mcp_defaults() {
    let (state, _rx) = start_mock_with(&["--mcp"]).await;
    assert!(!capabilities_of(&state).await.packetcode.mcp_defaults);

    let session = new_session_on(
        &state,
        ".",
        None,
        None,
        None,
        AcpMcpPosture::Explicit(vec![AcpMcpServer {
            name: "trusted-fs".into(),
            command: "/opt/fs-mcp".into(),
            args: Vec::new(),
            env: Default::default(),
...
/// The default-safe property at the transport level: `AcpMcpPosture::default()`
/// is what every caller that says nothing gets, and it starts nothing — even
/// against an engine that has a fleet on offer and would happily inherit it.
#[tokio::test]
async fn the_default_posture_starts_no_servers_on_a_fully_capable_engine() {
    let (state, _rx) = start_mock_with(&["--mcp", "--mcp-defaults"]).await;
    assert!(
        capabilities_of(&state).await.packetcode.mcp_defaults,
        "this engine WOULD inherit if asked"
    );

    let session = new_session_on(&state, ".", None, None, None, AcpMcpPosture::default())
        .await
        .unwrap();
    assert!(fleet_of(&state, &session).await.is_empty());
...
async fn concurrent_sessions_stream_independently_and_cancel_is_scoped() {
    ...
    let (state, mut rx) = start_mock().await;
    let a = new_session_on(&state, ".", None, None, None, AcpMcpPosture::None)
        .await
        .unwrap();
    let b = new_session_on(&state, ".", None, None, None, AcpMcpPosture::None)
        .await
        .unwrap();
    assert_ne!(a, b);
...
async fn graceful_stop_lets_the_engine_run_its_own_shutdown() {
    ...
    let session = new_session_on(&state, ".", None, None, None, AcpMcpPosture::None).await.unwrap();
...
async fn stop_kills_an_engine_that_ignores_stdin_close() {
    ...
    new_session_on(&state, ".", None, None, None, AcpMcpPosture::None).await.unwrap();
...
async fn abandoned_permission_waiter_is_reaped_when_its_turn_ends() {
    ...
    let (state, mut rx) = start_mock().await;
    let a = new_session_on(&state, ".", None, None, None, AcpMcpPosture::None).await.unwrap();
    let b = new_session_on(&state, ".", None, None, None, AcpMcpPosture::None).await.unwrap();
    assert_ne!(a, b);
...
///
///  1. The replay is REAL and it arrives BEFORE the load resolves — so a
///     client that opens its suppression window after awaiting the request has
///     already leaked the whole history into a live conversation.
///  2. The replay carries `user_message_chunk` updates, and the api-agent
///     event contract has no user-turn event to translate them into. Admitting
///     the rest anyway would paint every answer with its question missing.
///
/// `acp::events::translate_update` therefore emits nothing for the duration of
/// the window (see `the_load_replay_window_emits_nothing`), and the adopting
/// conversation says in words that the history lives in the engine. The
/// engine keeps it as the model's context regardless; only the rendering is
/// declined.
#[tokio::test]
async fn session_load_replays_a_transcript_that_cannot_be_rendered() {
    let (state, mut rx) = start_mock_with(&["--replay"]).await;
    let session = new_session_on(&state, ".", None, None, None, AcpMcpPosture::None)
        .await
        .unwrap();

    timeout(STEP, load_session_on(&state, &session, ".", AcpMcpPosture::None))
        .await
        .expect("session/load timed out")
        .expect("session/load failed");

    // Everything the mock replayed is already queued: it was sent before the
    // load's own response, so by the time the await above returned, a client
    // without a suppression window had nothing left to suppress.
    let mut replayed = Vec::new();
    while let Ok(event) = rx.try_recv() {
        match event {
            Event::Update(value) => replayed.push(value),
            other => panic!("expected session/update during replay, got {other:?}"),
        }
    }
    let kinds: Vec<&str> = replayed.iter().map(kind).collect();
    assert_eq!(
        kinds,
        vec![
            "user_message_chunk",
        ...
        "the replay must arrive in full, and in order, before the load resolves"
    );

    // The load-bearing half: the user's own turns are in the replay, and there
    // is no api-agent event they could become. Rendering the rest would be a
    // transcript with the questions cut out.
    assert_eq!(
        replayed.iter().filter(|u| kind(u) == "user_message_chunk").count(),
        2
    );
