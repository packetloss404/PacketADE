...
// --replay makes session/load behave the way a real engine does: the stored
// transcript is streamed back as session/update notifications BEFORE the load
// resolves. Two things about it matter to the client. It carries
// `user_message_chunk` updates, which the api-agent event contract has no
// event for; and nothing on the wire distinguishes a replayed update from a
// live one — only the client's own knowledge that a load is in flight does.
// That is exactly why `AcpSessions::set_replaying` brackets the request.
const withReplay = args.includes("--replay");
/** One session's stored history, in the order a real engine replays it. */
const replayTranscript = [
  { sessionUpdate: "user_message_chunk", content: { type: "text", text: "fix the router" } },
  { sessionUpdate: "agent_message_chunk", content: { type: "text", text: "Looking at it." } },
  {
    sessionUpdate: "tool_call",
    toolCallId: "replay-1",
    title: "Read",
    rawInput: { path: "router.go" },
  },
  {
    sessionUpdate: "tool_call_update",
    toolCallId: "replay-1",
    status: "completed",
    content: [{ type: "text", text: "package router" }],
  },
  { sessionUpdate: "agent_message_chunk", content: { type: "text", text: "Fixed." } },
...
function handleLine(line) {
  ...
  switch (method) {
    ...
    case "session/load": {
      ...
      // Replay first, THEN resolve — the ordering the spec requires and the
      // reason the client's replay window has to open before the request.
      if (withReplay) {
        for (const upd of replayTranscript) update(sessionId, upd);
      }
      send({ jsonrpc: "2.0", id, result: {} });
      break;
    }
