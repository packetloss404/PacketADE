...
| N5  | Cost alerts                      |       P2 | Partial     | Budget thresholds (daily / global-monthly / session / per-provider / per-flight) + warning thresholds + 24h overrides shipped in `src/lib/costGuardrails.ts` and `src/stores/costGuardrailStore.ts`, wired into the launch gate (`assertCostGuardrailsAllowLaunch` in `agentTaskStore` + `asyncFlightStore`) and surfaced in `LiveSpendChip` + `CostDashboardView`. Remaining work is alert/notification UX (toast/push on approaching a limit). See [`dev/moat/cost-dashboard-plan.md`](./dev/moat/cost-dashboard-plan.md). |
