...
>   tried to _refute_ it). A second throttled run refilled the 5 subsystem passes that the
...
These are the patterns that recur across the codebase. Fixing the _pattern_ matters more
...
1. **Spawn-and-forget: subprocesses are never reaped and exit codes are never captured.**
   ...
   without `wait()`/`kill_on_drop`. Worse, **orchestration decides task success by _whether_
   a process exited, not _how_ it exited** (G23, G24) — so a crashed/failed agent rolls the
   milestone up to **Done**. This is the single biggest blind spot: _the app cannot tell
   success from failure of a child process._
...
8. **Listeners/resources not cleaned up on terminal states.** F36 / G32 (12 listeners leaked
   per finished conversation — the _common_ case), F37, F38 (mic), G06, G26 (worktrees).
...
9. **Security: validate-but-don't-enforce, or unescaped interpolation.** F40 (web_fetch is an
   ...
   checksum). _(Note: F07/F08 SSH host-key pinning were **refuted** — pinning is genuinely
   enforced.)_
...
The debate confirmed these are _not_ real bugs (or are intended/protected behavior):
...
| **G32** | `deleteConversation` **leaks all 12 api-agent listeners** for done/failed conversations (the common case)                                                                                                                                 | `stores/agentTaskStore.ts:1142-1182`                           | Detach listeners unconditionally for `mode==="api"`; gate only the cancel/close backend calls on active/idle. _(supersedes F36)_          |
...
Status 2026-06-08 worktree: implemented, pending final validation/release. `pty:exit` now carries
`exitCode` and `terminated`, orchestrator PTY kills set `terminated`, and the frontend maps
non-zero/terminated exits to unsuccessful task completion (G23/G24). Attempts subscribe to
`api-agent:done`/`:error` and move to `reviewing`/`failed` without relying solely on the sentinel
(G25). Deploy has a wait thread plus bounded/EIO-aware reader handling (F13). Scheduler tick
failures are logged, counted, notified, and pause the loop after a threshold (F33). `update_task`
reports unsupported `target_spec` patches as `deferred_fields`, not landed updates (F34; actual
Task-level target persistence is still future work). Local sidecar child exits fan out a recoverable
`api-agent:error` and clear local ownership before restart (G02).
