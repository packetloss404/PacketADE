...
- `create_task(milestone_id, title, prompt, agent_id, target_spec)`
  ...
  and `claimed_paths` should list the repo-relative paths the executor is
  expected to edit so the dispatcher can block overlapping queued/running tasks.
...
### Transport _(validated 2026-05-14 by spikes)_
