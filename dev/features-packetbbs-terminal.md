...
Status: proposed and scoped; no PacketBench implementation has started

Product boundary: PacketBench remains the local desktop development environment. PacketBBS remains an independently deployed community BBS with its own callers, authentication, database, moderation, web terminal, and raw Telnet service.

## Objective

Give PacketBench users one deliberate way to reach a configured PacketBBS node without turning either application into a dependency of the other:

```text
Configure endpoint -> Probe health -> Open web or Telnet terminal -> Authenticate inside PacketBBS
```

The first version is a saved connection preset and launcher. It does not synchronize Packet accounts, inject credentials, mirror BBS messages, or give PacketBBS access to PacketBench workspaces.
...
- Public web URL, Telnet host, and Telnet port are deployment configuration, not values PacketBench should guess.

PacketBench must tolerate an older or unreachable node by reporting the failed probe and preserving the saved configuration. A failed health probe must not erase or rewrite the endpoint.
...
No username or password field belongs in this card. Authentication occurs inside PacketBBS. PacketBench must not store BBS credentials in frontend state, ordinary files, workspace DTOs, or command history.
...
If no supported Telnet client is present, show the exact `host:port`, offer Web mode, and leave installation to the user. PacketBench does not bundle PacketBBS or a background BBS daemon.
...
- Do not import PacketBBS caller identities into PacketBench or treat a successful BBS login as PacketBench authorization.
- Do not add automatic reconnect loops; reconnection is user-initiated and bounded.

## Persistence contract

Store non-secret connection metadata under PacketBench's centralized brand/storage conventions. Suggested versioned shape:

```ts
type PacketBbsConnectionV1 = {
  schemaVersion: "packetbench.packetbbs-connection/v1";
  label: string;
  webUrl: string;
  telnetHost: string;
  telnetPort: number;
...
| **PBI3** | Web launch                               | Configured web terminal opens through PacketBench's safe external-URL path with no privileged embedding                                        | Command/component tests plus manual Windows/macOS/Linux smoke                       | queued |
...
| **PBI5** | Cross-repository proof                   | A released PacketBench build probes and opens a tagged PacketBBS release over HTTPS and its public TCP proxy                                   | Versioned smoke transcript naming both commits/tags and public endpoints            | queued |

## Acceptance bar

The feature is complete only when a user can save one PacketBBS endpoint, obtain an honest health result, open the selected terminal mode, log in inside PacketBBS, close the pane without affecting the board, restart PacketBench, and repeat the launch without credentials having been stored or echoed by PacketBench.
...
Keep this feature behind the current PacketBench product decisions and release gates. When reprioritized, implement PBI1–PBI4 as one small local feature, then run PBI5 against an explicitly supplied PacketBBS deployment. Do not block PacketBench startup, ordinary terminal panes, or offline use on PacketBBS availability.
