...
| **S7** | Rename `target_id` → `server_id` across the wire | The in-flight back-compat `target_id` field name is migrated to `server_id` end-to-end, with a read shim for persisted data. | `lib/tauri.ts:1331-1336`; Rust DTO field; persisted-data read shim. | cargo check + vitest (shim reads old field). | M | ⏸️ **deferred — env-blocked.** The wire type is the ts-rs–generated `AttemptTargetDto` in `src/generated/tauri-schema.ts`; renaming `AttemptTarget::Ssh.target_id`→`server_id` requires regenerating it via `pnpm generate:tauri-schema` (`cargo test export_api_bindings --ignored`) and validating with `check:tauri-schema` — both need native-Windows test execution (host-blocked in WSL). Do on a native build: rename the Rust field with `#[serde(alias="target_id")]` (persisted read shim) + a Rust deser test, regen the schema, then sweep the ~14 `attempt.target.targetId` FE sites → `serverId`. |
...
| **S10** | SFTP / port-forward / file-size cap (Phase 4.3) | Lift/raise the 2 MB remote file cap with a streamed transfer (SFTP or chunked), and/or port-forward support. | `core/tool_runtime_ssh.rs:10` (cap); transfer path. | cargo check + size-cap test. | L | ⏸️ **deferred — needs a live remote to verify.** Streamed-transfer correctness (chunk boundaries, memory, reassembly of a >2 MB file via SFTP/`scp`/chunked base64) is only meaningfully validated against a real remote, which this env lacks (S11 blocker). Ledger's Deferred section already sequences the file cap before port-forward. Pick up with a host: implement chunked read/write, raise `MAX_FILE_SIZE`, add a size-cap + reassembly integration test. |
| **S11** | Live Codex-over-SSH smoke (P2) | One real remote host: remote Codex auth + installed sidecar, per `dev/sidecar-over-ssh-verification.md` step 12. **Environment-gated** — needs a configured SSH server (absent in the 2026-07-19 dev env). | `dev/sidecar-over-ssh-verification.md`. | Manual verification record (no code gate). | S | ⏸️ **env-gated — not run.** No SSH server configured in this dev env; the live remote smoke (verification step 12) requires one. Run manually when a remote host with Codex auth + installed sidecar is available. |

## Status (2026-07-25)

**Shipped & merged-pending-review:** S1 (process-tree kill), S2 (keyPath
hygiene), S3 (remote-git polish), S4 (cancel fingerprint symmetry), S5 (resume
live-config), S6 (clone-to-remote), S8 (portable realpath fallback). All gated
(cargo check --tests / vitest / lint / build) and committed on
`feat/ssh-remote-hardening`.

**Deferred — environment-blocked** (need a remote host / native ts-rs regen,
absent in this dev env): S7 (needs `generate:tauri-schema` on native Windows),
S9 (needs a Windows-OpenSSH host), S10 (needs a live remote for streamed-
transfer verification), S11 (needs a configured SSH server). Each row above
records the exact pick-up steps.
