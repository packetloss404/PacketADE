...
|---|---|---|---|---|---|---|
| **M1** | Wire IDF search into the Timeline box | Timeline search ranks by the existing IDF `relevanceScores` (best first) while keeping any substring hit — no result the old search found is lost. Blank query = identity. | Exported pure `searchMemoryEvents` in `memoryStore.ts`; wired into `MemoryView.tsx` `filtered` memo. | Vitest: 4 cases (blank identity, strong>weak rank, substring-only kept, non-match dropped). lint/build green. | S | ✅ closed 2026-07-24 |
| **M2** | Timeline **project** + **date-range** filter chips | A second Timeline filter row: date-window chips (All/24h/7d/30d) + per-project chips (basename labels), composing with the type chips + search. | Pure `filterMemoryEventsByScope` (`memoryStore.ts`); `MemoryView.tsx` state + `filtered` memo + `ScopeChip` row. | Vitest: 4 cases (default no-op, by-project, date-window, compose). lint/build green. | S | ✅ closed 2026-07-24 |
| **M3** | Export / import memory (JSON + Markdown) | Header actions export the `{events, patterns}` slice as JSON and as a readable Markdown digest; import merges a JSON export (dedup by id). | `memoryStore.ts` new `exportMemory`/`importMemory`; buttons in `MemoryView.tsx` header (~216-233). | Vitest: round-trip export→import is identity; Markdown formatter snapshot; import dedups. | S–M | ✅ closed 2026-07-24 |
| **M4** | "+ Add to memory" in more surfaces | The generic `captureManually` affordance (à la `InvestigationPanel`) is added to the flight coordination timeline row and the agent transcript. | `FlightsView.tsx` `TimelineRow`; `chat/MessageList.tsx` assistant row; new pure `lib/memoryCapture.ts` builders feed `captureManually`. | Vitest: pure payload builders (source/tags/summary/body). | S | ✅ closed 2026-07-24 |
| **M5** | Confidence auto-rerating on outcome | A pattern injected into a flight's brief has its `confidence` bumped on that flight's success / decayed on failure. | In-session provenance map (`recordInjectedPatterns`) fed from `composeAsyncLaunchPrompt`; pure `rerateConfidence`/`applyConfidenceRerate`; `adjustConfidenceForFlight` wired from `captureFlightCompletionOnTransition` (`asyncFlightStore.ts:568`). | Vitest: rerate math (bump/decay/clamp) + injected-only application. | M | ✅ closed 2026-07-24 |
| **M6** | Recurring-error "this looks familiar" hint | At launch, if the prompt/flight matches a prior `pitfall` pattern or a repeated failure signature, surface a one-line hint. | Pure `selectRecurringErrorHint` (`lib/recurringErrorHint.ts`) over pitfall patterns + `flight_completed` lessons (recurrence-weighted); amber hint under the prompt in `LaunchAsyncFlightModal`. | Vitest: pitfall/failure match, recurrence count, threshold, ranking (pure). | M | ✅ closed 2026-07-24 |
| **M7** | 30-day memory digest | A digest summarizing the last 30 days of memory (counts by type/category + top patterns/lessons), shown in MemoryView. | Pure `computeMemoryDigest` (`lib/memoryDigest.ts`) over `events`/`patterns`; `MemoryDigestCard` at the top of the Patterns right rail. | Vitest: windowing, distinct-lesson collection, top-pattern ranking, empty. | M | ✅ closed 2026-07-24 |
| **M8** | "Ask your project" memory chat tab | A 3rd MemoryView tab: type a question → keyword-ranked memory answer (reuse `relevanceScores`; no LLM required for v1). | New `"ask"` `Tab` + `AskTab` panel in `MemoryView.tsx`; query plumbed straight into the exported `computeContextItems` ranker. | Vitest: query pulls the relevant pattern to the top (memoryStoreSettings). | M | ✅ closed 2026-07-24 |
| **M9** | Use the rich flight retrospective | Flight capture enriches the mechanical `flight_completed` payload with the `summarize_flight` LLM retrospective when learning is enabled (best-effort, local flights only). | New `summarizeFlight` `tauri.ts` binding + `FlightSummaryInput` camelCase rename; pure `lib/flightRetrospective.ts` (input builders + parser); `updateFlightRetrospective` store action; fire-and-forget `enrichFlightRetrospective` from the completion transition. | Vitest: pure builders/parser + store merge; cargo check the binding. | M | ✅ closed 2026-07-24 |
| **M10** | Resolve the dead `task_completed` path | **Decision: retire, not revive** — no real signal exists (scheduler removed July 2026). Dropped the permanently-empty "Tasks" filter chip; kept the type + `TaskCard` renderer read-only for legacy events. | Extracted `TIMELINE_FILTERS` to `memory/timelineFilters.ts` (no task chip); legacy comments on the union + `TaskCard`. | Vitest: `TIMELINE_FILTERS` excludes `task_completed`, keeps the live types. | S | ✅ closed 2026-07-24 |
