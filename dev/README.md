...
| [`acceptance.md`](./acceptance.md)                             | **ACTIVE CHECKLIST** - the packaged acceptance matrix, retargeted 0.11.0 -> 0.12.0 -> 0.12.1 -> 0.13.0 -> 0.13.1 as each build was superseded. **Filename is stable - retarget it in place, do not rename it;** the old per-version names broke every inbound link. Sections 0 (build), 1 (migration) and 2 (launch/lifecycle/shell) have run against installed 0.13.1 - three defects found across them, plus two partial rows and one blocked row. Sections 3-5 (dictation on real hardware, analytics, Monitor) still need a person at the keyboard with the headset attached |
