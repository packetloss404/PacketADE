...
| [`acceptance.md`](./acceptance.md)                             | **ACTIVE CHECKLIST** - the packaged acceptance matrix, retargeted 0.11.0 -> 0.12.0 -> 0.12.1 -> 0.13.0 -> 0.13.1 as each build was superseded. **Filename is stable - retarget it in place, do not rename it;** the old per-version names broke every inbound link. Sections 0 (build) and 1 (migration) have run and found two defects; section 2's cold-start row closed on 0.13.1, the first build whose UI anyone looked at. The rest of sections 2-5 need a person at the keyboard with the headset attached |
