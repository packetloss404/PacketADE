...
| Syndicate target            | [`syndicate-execution-target.md`](./syndicate-execution-target.md)                                 | **PUBLIC INFRA LIVE** - PacketADE flagship target, pairing/scopes/revoke, Host Workspaces, durable panes/replay, pinned-SSH bootstrap, encrypted PacketRelay, and isolation pass; signed v0.1.3 installer and WSS route are live; packaged real-host matrix remains |
| Syndicate device→relay spec | [`controller-protocol-device-relay-half.md`](./controller-protocol-device-relay-half.md)           | PacketADE's contribution to Syndicate's `CONTROLLER_PROTOCOL_V1`, which documents the controller→Host leg only. We own the only device-half implementation. Syndicate integrates the final document                                                                 |
