...
a task, role, attempt, or the whole Flight. PacketADE records delivery and
...
- PacketADE does not silently type into an interactive PTY. PTY agents receive
  inbox visibility through the PacketADE MCP provider when connected, with a
...
| **CI5** | MCP inbox surface | Extend the PacketADE MCP provider with scoped inbox reads, acknowledgement, and validated message posting behind `allow_writes`. Preserve bearer/origin/audit controls. | Rust MCP tool/resource and permission tests | CI2 | closed |
