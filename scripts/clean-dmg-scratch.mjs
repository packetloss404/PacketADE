 ...
 * image — `rw.<pid>.PacketBench_<ver>_<arch>.dmg` — mounts it, copies the app in,
 ...
 * PacketBench scratch images and delete leftover `rw.*.dmg` files. macOS-only;
...
// 1. Detach any attached PacketBench scratch images. `hdiutil info` groups each
...
//    flag a block when its image-path matches a PacketBench scratch image, then
...
for (const line of info.split("\n")) {
  ...
  if (/image-path\s*:.*rw\.\d+\.PacketBench/i.test(line)) {
    blockIsOurs = true;
    continue;
  }
...
try {
  for (const name of readdirSync(macosBundleDir)) {
    if (/^rw\.\d+\.PacketBench.*\.dmg$/i.test(name)) {
